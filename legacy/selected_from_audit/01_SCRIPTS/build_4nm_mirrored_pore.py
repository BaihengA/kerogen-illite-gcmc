...
# -*- coding: utf-8 -*-
"""
V32 - Build a 4 nm mirrored slit pore from the exact kerogen+illite composite wall.
...
def main() -> None:
    ...
    ap.add_argument("--config", default="4nm_pore_raspa2_config.json")
    ...
    for case in cases:
        ...
        pore_gro = od / f"{case}_COMPOSITE_PORE_4nm.gro"
        write_gro(pore_gro, f"{case} mirrored composite slit pore nominal {pore_nm:.3f} nm", combined, box)
        write_geometry_cif(od / f"{case}_COMPOSITE_PORE_4nm_GEOMETRY_ONLY.cif", f"{case}_COMPOSITE_PORE_4nm", combined, box)
    ...
    if all_reports:
        ...
        with (out_root / "PORE_4NM_BUILD_REPORT_ALL.csv").open("w", newline="", encoding="utf-8-sig") as f:
